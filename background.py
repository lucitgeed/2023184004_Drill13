...
class FixedBackground:
    ...
    def __init__(self):
        ...
        self.w, self.h = self.image.w, self.image.h             #백그라운드의 너비와 높이의 정보를 이곳에 넣음
...
#        self.image.draw(get_canvas_width()//2, get_canvas_height()//2)
    ...
    def update(self):
        ...
        self.window_left = clamp(0,
                                 int(server.boy.x) - self.cw//2,            #server.boy.x는 실수이기때문에 정수로 변경
                                 self.w - self.cw -1
                                 )
...
class TileBackground:
    ...
    def draw(self):
        ...
        ##offset을 픽셀단위로 하기위해 % 600, % 800으로 나원야함.
